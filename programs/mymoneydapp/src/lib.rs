...
pub enum AuthorityType {
    ...
    FreezeAccount, // Auth to free any account associated w/the Mint
    AccountOwner,  // Owner of a given token account
    CloseAccount   // Auth to close a token account
}

/*
The Accounts macro implements the Accounts trait.
Transforms a struct from the untrusted &[AccountInfo] slice given to a Solana progam 
into a validated struct of deserialized account types
*/
#[derive(Accounts)]
pub struct ProxyTransfer<'info> {
    #[account(signer)]
    pub authority: AccountInfo<'info>,  // Authority of account must sign
    #[account(mut)]
    pub from: AccountInfo<'info>,
    #[account(mut)]
    pub to: AccountInfo<'info>,
    pub token_program: AccountInfo<'info>,  // token program to call
}
...
pub struct ProxyMintTo<'info> {
    ...
    #[account(mut)]
    pub mint: AccountInfo<'info>,   // the account of our token (supply)
    #[account(mut)]
    pub to: AccountInfo<'info>,
    pub token_program: AccountInfo<'info>,  // token program to call
}
